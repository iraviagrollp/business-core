...
# [0]=Date, [1]=Voucher No, [2]=Branch, [5]=Party, [6]=Ref BillNo,
# [9]=Product, [10]=Qty, [25]=Barcodes
...
def _process(bucket: str, key: str, filename: str):
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            ...
            logger.info('Parsed %d purchase ledger rows', len(rows))
            _upsert(conn, rows)
            conn.commit()
...
def _parse(src_path: str, barcode_dates: dict) -> list[dict]:
    ...
    for row in ws.iter_rows(min_row=6, values_only=True):
        ...
        # Technical name: the product field is a CSV — strip all commas
        technical_name = str(row[9] or '').replace(',', '').strip()
        ...
        rows.append({
            ...
            'supplier_voucher': str(row[6] or '').strip() or None,
            'branch': str(row[2] or '').strip() or None,
            'party': str(row[5] or '').strip() or None,
