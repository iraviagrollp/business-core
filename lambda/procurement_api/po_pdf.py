...
tint #f7f9f7, peach note #fdf6ef. DejaVuSans is bundled so the ₹ glyph renders; if the
fonts are missing it degrades to Helvetica + "Rs.".
"""
...
_DESC_SUB = 'Technical grade standards & batch-wise COA to accompany invoice'
...
def _register_fonts():
    """Register bundled DejaVu (₹-capable). Returns (base, bold, rupee) — falls back to
    Helvetica + 'Rs.' if the TTFs are absent."""
    try:
        if 'DejaVuSans' not in pdfmetrics.getRegisteredFontNames():
            pdfmetrics.registerFont(TTFont('DejaVuSans', os.path.join(_DIR, 'DejaVuSans.ttf')))
            pdfmetrics.registerFont(TTFont('DejaVuSans-Bold', os.path.join(_DIR, 'DejaVuSans-Bold.ttf')))
...
def _styles():
    def s(name, size, **kw):
        ...
        kw.setdefault('leading', size * 1.18)
        return ParagraphStyle(name, fontSize=size, **kw)
    return {
        'company': s('company', 19, fontName=_BOLD, textColor=_GREEN, alignment=TA_CENTER, leading=22),
        'tagline': s('tagline', 10.5, fontName=_BOLD, textColor=_ORANGE, alignment=TA_CENTER),
        'identity': s('identity', 8.5, textColor=_MUTED, alignment=TA_CENTER),
        'potitle': s('potitle', 19, fontName=_BOLD, textColor=_GREEN, leading=22),
        'seclabel': s('seclabel', 9.5, fontName=_BOLD, textColor=_GREEN),
        'boxlabel': s('boxlabel', 8.5, fontName=_BOLD, textColor=_GREEN),
        'boxval': s('boxval', 9.5, textColor=_BODY),
        'name': s('name', 12, fontName=_BOLD, textColor=_GREEN),
        'body': s('body', 9.5, textColor=_BODY),
        'bodyb': s('bodyb', 9.5, fontName=_BOLD, textColor=_BODY),
        'hi': s('hi', 9.5, fontName=_BOLD, textColor=_GREEN),
        'th': s('th', 8.5, fontName=_BOLD, textColor=colors.white),
        'thr': s('thr', 8.5, fontName=_BOLD, textColor=colors.white, alignment=TA_RIGHT),
        'thc': s('thc', 8.5, fontName=_BOLD, textColor=colors.white, alignment=TA_CENTER),
        'cell': s('cell', 9, textColor=_BODY),
        'cellc': s('cellc', 9, textColor=_BODY, alignment=TA_CENTER),
        'cellr': s('cellr', 9, textColor=_BODY, alignment=TA_RIGHT),
        'prod': s('prod', 10, fontName=_BOLD, textColor=_GREEN),
        'sub': s('sub', 8, textColor=_MUTED),
        'words': s('words', 9.5, textColor=_BODY),
        'tot': s('tot', 11, fontName=_BOLD, textColor=colors.white),
        'totr': s('totr', 11, fontName=_BOLD, textColor=colors.white, alignment=TA_RIGHT),
        'addr': s('addr', 9, textColor=_BODY),
        'ctlabel': s('ctlabel', 9, fontName=_BOLD, textColor=_GREEN),
        'tc': s('tc', 7.5, textColor=_MUTED, leading=9),
        'note': s('note', 9, textColor=_BODY),
        'sign': s('sign', 9.5, textColor=_BODY),
        'signr': s('signr', 9.5, textColor=_BODY, alignment=TA_RIGHT),
        'signrb': s('signrb', 10, fontName=_BOLD, textColor=_GREEN, alignment=TA_RIGHT),
        'signrs': s('signrs', 8, textColor=_MUTED, alignment=TA_RIGHT),
    }
...
def _addr_para(po, prefix, st):
    ...
    lines = [f'<font name="{_BOLD}" color="#17452f">{_esc(name)}</font>']
...
def render_po_pdf(po: dict) -> bytes:
    ...
    desc = [Paragraph(_esc(po.get('technical_name')), st['prod']), Paragraph(_DESC_SUB, st['sub'])]
    ...
    words_cell = [Paragraph('TOTAL ORDER VALUE IN WORDS', st['boxlabel']), Spacer(1, 3),
                  Paragraph(_amount_in_words(total), st['words'])]
    right = Table(
        [[Paragraph('Taxable Value', st['body']), Paragraph(_inr(amount), st['cellr'])],
         [Paragraph(f'GST @ {gst_lbl}%', st['body']), Paragraph(_inr(gst_amt), st['cellr'])],
    ...
    flow.append(Spacer(1, 3))
    bs = Table(
        [[Paragraph('BILL TO', st['boxlabel']), Paragraph('SHIP TO', st['boxlabel'])],
    ...
    ctd = [[Paragraph(lbl, st['ctlabel']), Paragraph(_esc(val) if val else '&mdash;', st['cell'])] for lbl, val in ct]
