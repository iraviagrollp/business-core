"""
customer_balances_fy — shared Customer Balances (FY) computation.
...
compute_customer_balances_fy(conn, fy_count) -> dict
    ...
    returns  : payload dict matching the GET /reports/customer-balances-fy JSON
               contract (fys, rows, totals)

Shared by
---------
  lambda/api/handler.py             — GET /reports/customer-balances-fy thin
                                      wrapper (cache-aside + _response)
  lambda/alerts_evaluator/handler.py — customer_balances_fy alert branch (PDF
                                      email path; always calls with fy_count='all')

Keep lambda/alerts_evaluator/customer_balances_fy.py byte-identical to this file
...
Per-voucher netting (roundoff absorption):
  ...
  is_cn (credit-note) vouchers -> credit_notes += -net
  net > 0                       -> debit  += net
  net < 0                       -> credit += -net
  net == 0                      -> nothing

Credit-note identification:  sub_category == _CREDIT_NOTE_SUBCATEGORY
...
# Sub-category string that marks credit-note transactions in customer_ledger.
# Change here (and flush Redis) if the value ever changes in the ledger ETL.
...
def compute_customer_balances_fy(conn, fy_count) -> dict:
    """Compute per-customer, multi-FY roll-forward of debits/credits.

    Parameters
    ----------
    conn     : open psycopg2 connection — caller is responsible for lifecycle.
    fy_count : 'all'  — show every FY present in customer_ledger; zero opening.
               int>=1 — show the most recent N FYs; first shown FY gets a
                        brought-forward opening from all transactions before it.

    Returns
    -------
    dict matching the GET /reports/customer-balances-fy JSON contract:
      {
        'fys':  [fy_label, ...],           # oldest -> newest
        'rows': [{party, code, city, opening, per_fy, balance_dr, balance_cr}],
    ...
    # ── 2. City and code lookup from customer_details ──────────────────────────
    with conn.cursor() as cur:
        cur.execute("""
    ...
    agg: dict = defaultdict(lambda: defaultdict(
        lambda: {'debit': 0.0, 'credit': 0.0, 'credit_notes': 0.0}
    ))
    ...
    def _party_sort_key(party: str) -> tuple:
    ...
    # Compute running totals balance per FY (net = debit - credit - credit_notes)
    for label in shown_fy_labels:
    ...
    logger.info(
        'compute_customer_balances_fy: fy_count=%s fys=%d parties=%d',
