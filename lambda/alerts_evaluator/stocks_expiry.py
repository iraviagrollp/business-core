...
def compute_stocks_expiry(conn, branch=None, expires_before_months=None, brand=None) -> dict:
    ...
    # expires_before_months may arrive as a query-param string ('3'/'6'/'9'/'12',
    # from GET /stocks/expiry/pdf) OR as a float (3.0/6.0/9.0/12.0, from an
    # alert_conditions.value — alerts_eval always stores/loads condition values
    # as float). Normalise both forms to an int and validate against the same
    # {3, 6, 9, 12} set either way.
    months_int = None
    if expires_before_months not in (None, ''):
        try:
            months_int = int(float(expires_before_months))
        except (TypeError, ValueError):
            months_int = None

    cutoff_date = None
    if months_int in (3, 6, 9, 12):
        today = datetime.now(timezone.utc).date()
        cutoff_date = _add_months(today, months_int)
