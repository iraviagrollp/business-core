...
- **New `GET /stocks/expiry/pdf`** (api Lambda, `lambda/api/stocks_expiry_pdf.py`) — computed fresh (no Redis cache, matches the other report-PDF routes' convention), landscape A4, shared `letterhead.py` header/footer repeating every page. Query params: `brand` (case-insensitive substring, `ILIKE '%…%'`), `branch` (**added 2026-08-01** — EXACT match, `AND branch = %s`, same convention as `/sales/list`/`/purchases/list`'s `branch` param, not a substring; absent/empty `branch` = no branch filter, handles both `None` and `''` defensively), and `expires_before_months` (`3`/`6`/`9`/`12`, else no filter). Month filter computed via a small `_add_months(date, months)` helper in `handler.py` (calendar-safe day clamping, e.g. 31-Jan + 1 → 28/29-Feb) — `expiry_date <= today + N months`; rows with NULL expiry are EXCLUDED when the month filter is active, INCLUDED when it is absent. All three filters AND together. Columns: Brand, Technical, Packing, Branch, Special Packing, Available Nos, Conversion Factor, Available Cases, Available Qty, Entry Date, Expiry Date — NO rate/valuation. Subtitle line shows active filters in order Branch → Brand → cutoff, e.g. `Branch: Auto Nagar · Brand: GULFONID · Expiring before 21-11-2026`, or `All Stock` when none — `stocks_expiry_pdf.py`'s `render_stocks_expiry_pdf(data)` now reads an added `data['branch_filter']` key (payload dict gained this key; `rows`/`brand_filter`/`cutoff_date` keys unchanged). Filename gained a leading branch segment: `stock_expiry_<branch>_<brand>_<months>.pdf` (each part `_safe_filename_part`-sanitized, `"all"` when unset — was `stock_expiry_<brand>_<months>.pdf` before). `GET /stocks/expiry` (JSON) is UNCHANGED — still returns all rows with no branch param; the UI filters that endpoint client-side.
...
- `issued_pdc_pdf.py::render_issued_pdc_pdf(data) -> bytes` — standalone (not shared, only one caller) 16-column landscape-A4 renderer for Issued PDC; TOTAL row sums Gross/GST/Amount/Disc/Adv/Bal/PDC Amt. **Reworked 2026-08-05** (see the dedicated bug-fix entry below the routes list) — column widths are now measured per render against the actual payload (`_COL_SPECS`/`_measure()`/`_compute_layout()`), with a padding/font/margin degradation ladder, instead of guessed fixed weights; the 7 money columns no longer carry a per-cell `₹` glyph (stated once in the subtitle instead).
...
**Bug fix (2026-08-05) — Issued PDC PDF cell overflow, dynamic measured column widths:** the 2026-08-05 initial ship of `issued_pdc_pdf.py` (line above, and the `/pdc/pdf` route entry above) had a REAL user-reported rendering bug: the Date column and several money columns overflowed their cell boundaries (text spilling into the neighbouring column) at real data widths (dates `05-08-2026`, lakh/crore-scale ₹ amounts) even though every cell was already a `Paragraph` — the previous "smoke-tested, no exceptions, N pages" verification (line above) proved nothing about layout, since a non-empty `%PDF` with a plausible page count says nothing about whether text stayed inside its cell. Root cause: dates/amounts are single unbreakable tokens (no spaces) and the old fixed-weight `_COLUMNS` split (guessed weights, never measured against `pdfmetrics.stringWidth`) under-allocated them. Fix, entirely in `issued_pdc_pdf.py` (no other file touched):
  - **Per-cell `₹` glyph dropped from the 7 money columns** — `_fmt_inr`/`_amt` no longer prepend `_RS`; the money column headers are now bare `Gross`/`GST`/`Amount`/`Disc`/`Adv`/`Bal`/`PDC Amt` (no `(₹)` suffix); the unit is instead stated once, in the subtitle line, as `All amounts in {_RS}` (still routed through the `_RS` inline-font token, per house convention — a bare `₹` in a Helvetica Paragraph KeyErrors). Reclaims ~4-5pt per money cell × 7 columns.
  - **Column widths are now MEASURED PER RENDER, not guessed weights.** New `_COL_SPECS` (16-entry list: key/header/align/`wrap`/`money`/`cell(row)->str`) + `_measure()`/`_compute_layout()`: for the 13 unbreakable columns (PO, Date, PDC Date, Cr.Days, Qty, Rate, the 7 money columns), the column gets `max(pdfmetrics.stringWidth(header), every data cell, TOTAL cell) + 2×padding` for THIS specific payload's actual rows — always ≥ what it needs, whether the amounts are tiny or crore-scale, negative, or `None`. Supplier/Product/Brand (the 3 genuinely wrappable columns) get a measured MINIMUM (`max(header, longest single WORD)` — the true wrap floor) and then split whatever width is left over, proportional to their average full-cell width (so Product — usually the longest text — gets the biggest share).
  - **Graceful degradation ladder** (`_DEGRADE_STEPS`, re-measured at each step until everything fits): cell padding 3pt→2pt, body/header/total font 7pt→6.5pt→6pt (now uniform across header/data/TOTAL — was 6.5/7/7 before), side margin 1.0cm→0.75cm. On real Issued PDC data this rarely needs to go past pad=2/font=7 or occasionally font=6; most payloads tested (including full crore-scale) fit at the BEST-looking tier (7pt/3pt-padding/1cm-margin) untouched.
  - **Last-resort "genuinely unfittable" path** (only reachable with a pathological single unbreakable token ~30+ chars with zero spaces in Supplier/Product/Brand — not realistic data): the 13 unbreakable columns keep their own required width, the 3 wrap columns are clamped to their hard minimum (not shrunk further), and a `logging.warning()` (key: `issued_pdc_pdf`) records the table-vs-page overflow amount and the columns involved — an intentional, reported, bounded trade-off, never a silent overflow.
  - **Verification (this fix, scratchpad-only script, not committed):** a geometric assertion — `stringWidth(cell_text, cell_font, cell_font_size) <= col_width - 2×padding` for the 13 unbreakable columns, `stringWidth(longest_word, ...) <= col_width - 2×padding` for the 3 wrap columns — run against every header/data/TOTAL cell for 8 worst-case payloads (crore-scale in all 7 money columns × 50 rows, negative amounts, long-but-realistic PO/company/technical/brand text, a deliberately pathological 40-char no-space single word in all 3 wrap columns, all-`None` rows, an 80-row TOTAL-row stress case reaching ~₹1,17,99,999.99×80, an empty `rows` list, and a normal small-scale case): **4184 cells checked, 0 failures** on the 7 realistic payloads; the 1 deliberately pathological payload hit the documented last-resort path (0 per-cell failures — the 40-char token exactly fills its own clamped-minimum column — but the table itself is ~99pt wider than the page, logged as designed, not silently shipped). Also re-rendered every payload end-to-end (`render_issued_pdc_pdf`) and visually inspected two pages of a 31-row crore-scale + 1 negative-with-`None`-PO/company sample via PyMuPDF: no overflow anywhere, TOTAL row (10-crore-scale sums) renders cleanly, subtitle shows `All amounts in ₹ | Supplier: … | PDC Period: …`. `python -m py_compile` clean on `issued_pdc_pdf.py`; no other file changed (route, filename, column set/order, TOTAL row semantics, letterhead/zebra/GREEN-header conventions all unchanged).
  - **Sibling renderers checked, NOT changed:** `aging_pdf.py` (Customer/Supplier Aging, 9 columns) and `transactions_register_pdf.py` (Sales/Purchases Register, 9 columns) render the same kind of unbreakable ₹/date tokens in fixed-weight columns, so the same class of bug was plausible. Ran the identical geometric-assertion methodology (distinguishing each renderer's genuinely wrappable text columns — Party/Supplier, City, Product, Branch — from its unbreakable columns, and correctly parsing the `_RS` inline-font markup width instead of measuring the raw markup characters) against equally nasty worst-case payloads (crore-scale × 60 rows, negative amounts, long-but-realistic party/product/voucher text, `None` rows, empty `rows`, TOTAL-row stress): **0 failures on all realistic worst-case payloads**, with 24-38pt of comfortable slack per money column at crore scale (e.g. a crore-scale amount measures ~47.5pt in a column with ~71-85pt available). The only failures were from two payloads deliberately crafted to be unrealistic beyond even the task's stress-test intent (a 43-character city name and a 37-character voucher number, both with zero internal spaces) — real city names and FUSIL-generated voucher numbers (`POSRT2526-7`, `IAL/2627/999999-REV`, etc., verified up to ~20 chars) fit comfortably. Per the task's own instruction not to make a cosmetic change to a working report, **both files were left untouched** — they already have genuine slack, not a latent version of this bug.
...
- [x] `GET /stocks/expiry/pdf` gained an EXACT-match `branch` query param (2026-08-01, follow-up
  to the Stock Expiry tracking task above — see the full write-up in the `GET /stocks/expiry/pdf`
  bullet under "etl_stocks — Stock Expiry tracking" for filter/filename detail). `handler.py`'s
  `_handle_stocks_expiry_pdf` ANDs `branch = %s` alongside the existing `brand ILIKE`/cutoff-date
  filters; `stocks_expiry_pdf.py`'s subtitle now shows `Branch: <name> · Brand: <name> · Expiring
  before <date>` (branch first) or `All Stock`; filename gained a leading branch segment. `GET
  /stocks/expiry` (JSON) is unchanged — no branch param, UI still filters it client-side. Verified:
  `python -m py_compile handler.py stocks_expiry_pdf.py` clean; `render_stocks_expiry_pdf` smoke-
  tested (temp script, deleted with `__pycache__` after) for branch+brand+cutoff, no filters
  ("All Stock"), branch-only, zero-matching-rows, and brand+cutoff-without-branch — every call
  returned valid `%PDF`-prefixed bytes (~214–216 KB); `pypdf` text extraction on each case
  confirmed the exact expected subtitle line (branch ordered first, correct separator, correct
